...
const [datosInforme, setDatosInforme] = useState<{
  votos: Record<string, { aFavor: number; enContra: number; abstencion: number }>;
}>({ votos: {} });
...
const limpiarFormularioPunto = () => {
  ...
  const prepararDatosInforme = async () => {
    ...
    try {
      ...
      const conteoVotos: Record<string, { aFavor: number; enContra: number; abstencion: number }> = {};
      ...
      setMensaje("Los datos reales de la sesión fueron preparados correctamente para el informe.");
    } catch (error) {
      console.error("Error preparando datos del informe:", error);
      setError("No fue posible preparar los datos del informe.");
    } finally {
...
<div style={seccionStyle}>
  ...
  >
    <div>
      ...
      <p style={textoSecundario}>
        Prepare el informe a partir de la información registrada en esta sesión.
      </p>
    </div>

    <span
      style={{
        background: informePreparado ? "#dcfce7" : "#fef3c7",
        color: informePreparado ? "#166534" : "#92400e",
        padding: "7px 13px",
        borderRadius: 999,
        fontSize: 13,
        fontWeight: "bold",
    ...
    >
      {informePreparado ? "PREPARADO" : "BORRADOR"}
    </span>
  </div>
  ...
  >
    <strong style={{ color: "#111827" }}>Anexos opcionales</strong>
    ...
    >
      <label style={{ display: "flex", gap: 10, alignItems: "center", color: "#374151" }}>
        <input
          type="checkbox"
          checked={incluirAsistencia}
          onChange={(e) => setIncluirAsistencia(e.target.checked)}
        />
      ...
      <label style={{ display: "flex", gap: 10, alignItems: "center", color: "#374151" }}>
        <input
          type="checkbox"
          checked={incluirVotaciones}
          onChange={(e) => setIncluirVotaciones(e.target.checked)}
        />
      ...
      <label style={{ display: "flex", gap: 10, alignItems: "center", color: "#374151" }}>
        <input
          ...
          onChange={(e) => setIncluirAcuerdos(e.target.checked)}
        />
        Detalle de acuerdos
      </label>
      ...
      <label style={{ display: "flex", gap: 10, alignItems: "center", color: "#374151" }}>
        <input
          type="checkbox"
          checked={incluirOtrosAnexos}
          onChange={(e) => setIncluirOtrosAnexos(e.target.checked)}
        />
        Otros anexos
      </label>
    </div>
  ...
  >
    <button
      ...
      style={{
        ...botonPrimario,
        opacity: cargandoInforme ? 0.7 : 1,
      }}
    >
      {cargandoInforme ? "Preparando datos..." : "🤖 Generar informe con IA"}
    </button>

    <button
      type="button"
      disabled={!informePreparado}
      ...
      style={{
        ...
        background: "#fff",
        color: "#374151",
        cursor: informePreparado ? "pointer" : "not-allowed",
        fontWeight: "bold",
        opacity: informePreparado ? 1 : 0.55,
      }}
    >
  ...
  {mostrarInforme && informePreparado && (
    <div
      style={{
        marginTop: 22,
        padding: 22,
    ...
    >
      <h3 style={{ marginTop: 0, color: "#111827" }}>Vista previa de los datos del informe</h3>

      <div style={{ display: "grid", gap: 12, color: "#374151", lineHeight: 1.6 }}>
        <div><strong>Sesión:</strong> {sesion.titulo}</div>
        <div><strong>Fecha:</strong> {fechaTexto}</div>
        <div><strong>Lugar:</strong> {sesion.lugar}</div>
        <div><strong>Asistencia registrada:</strong> {totalAsistencias} persona(s)</div>
        <div><strong>Puntos tratados:</strong> {puntos.length}</div>
        <div><strong>Votaciones:</strong> {votaciones.length}</div>
        <div><strong>Acuerdos:</strong> {acuerdos.length}</div>

        {votaciones.length > 0 && (
          <div style={{ marginTop: 6 }}>
            <strong>Resultados de las votaciones</strong>
            <div style={{ marginTop: 8, display: "grid", gap: 10 }}>
              {votaciones.map((votacion, index) => {
                const conteo = datosInforme.votos[votacion.id] || { aFavor: 0, enContra: 0, abstencion: 0 };
                return (
                  <div
                    key={votacion.id}
                    style={{
                  ...
                  >
                    <div><strong>#{index + 1}</strong> {votacion.pregunta}</div>
                    <div style={{ marginTop: 4, fontSize: 14, color: "#6b7280" }}>
                      A favor: {conteo.aFavor} · En contra: {conteo.enContra} · Abstenciones: {conteo.abstencion}
                    </div>
                  </div>
                );
              })}
        ...
        >
          ℹ️ Esta vista previa utiliza únicamente datos registrados en RENALIX. La generación del texto con IA y el PDF se implementarán en los siguientes pasos.
        </div>
      </div>
    </div>
  )}
